...
(async () => {
  ...
  (function startEmojiDust() {
    ...
    for (let i = 0; i < 18; i++) {
      ...
      span.style.animation = `float${i % 3} ${
        18 + Math.random() * 12
      }s linear infinite`;
  ...
  async function initLeafletMap(list) {
    ...
    for (const p of list) {
      ...
      const m = L.marker(latlng, { icon: emojiIcon(p.emoji || "📍") }).addTo(
        map
      );
      const popupHtml = `
  ...
  function renderFood(list) {
    const host = document.getElementById("foodList");
    if (!host) return;
    host.innerHTML = "";
    ...
    list.forEach((place) => {
      ...
      const checked = place.visited === true;
      const line2 = checked
        ? `Visited on ${place.date || ""}`.trim()
        : "coming soon";

      li.innerHTML = `
      <div class="food-checkbox ${checked ? "checked" : ""}">
        ${checked ? "✓" : ""}
      </div>
      <div class="food-main">
        <div class="food-title">
          <strong>${place.name}</strong> <span class="food-loc">(${
        place.location
      })</span>
        </div>
        <div class="food-note">${line2}</div>
      </div>
    `;
  ...
  function save() {
    localStorage.setItem(
      ...
      JSON.stringify({
        order,
        revealed: [...document.querySelectorAll(".card.revealed")].map(
          (c) => +c.dataset.idx
        ),
      })
    );
  ...
  function load() {
    ...
    try {
      ...
      if (Array.isArray(revealed))
        revealed.forEach((i) => revealByIndex(i, false));
      updateProgress();
    } catch {
      build();
    }
      ...
      card.innerHTML = `
        <div class="inner">
          <div class="face front">
            ...
            <div class="num">${n + 1} <span class="emoji">${
  ...
  paceScrim.addEventListener("click", (e) => {
    e.stopPropagation();
    closePace();
  });
  paceOk.addEventListener("click", (e) => {
    e.stopPropagation();
    closePace();
  });
  ...
  [
    ["d_revealOne", handleRevealOne],
    ["d_revealAll", handleRevealAll],
    ["d_resetAll", handleReset],
    ["d_shuffle", handleShuffle],
    ["d_pace", handlePace],
    ["m_revealOne", handleRevealOne],
    ["m_revealAll", handleRevealAll],
    ["m_resetAll", handleReset],
    ["m_shuffle", handleShuffle],
    ["m_pace", handlePace],
